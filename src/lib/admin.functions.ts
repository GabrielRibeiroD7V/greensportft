...
export const getAdminStats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    // Use the authenticated user's client from context (RLS-scoped)
...
export const getRiskExposure = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }) => {
    await assertAdmin(context.supabase as any, context.userId);
    // Exposure by Fixture
    const { data: fixtureExposure } = await context.supabase
      .from("betting_tickets")
      .select(`
        potential_return,
        stake,
