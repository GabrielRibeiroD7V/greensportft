...
function AdminMappingsPage() {
  ...
  const filteredMarkets = data.markets?.filter((m: any) => 
    m.internal_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50 hover:bg-slate-50 border-slate-200">
                <TableHead className="font-black text-slate-500 uppercase text-[10px]">Internal Name</TableHead>
                <TableHead className="font-black text-slate-500 uppercase text-[10px]">Provider Name</TableHead>
            ...
            <TableBody>
              {filteredMarkets?.map((market: any) => (
                <TableRow key={market.id}>
                  <TableCell className="font-bold text-slate-700">{market.internal_name}</TableCell>
